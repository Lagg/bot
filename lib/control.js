...
Control.prototype._evalLine = function (line) {
    ...
    } else if (parsedCmdArgs.length < command.requiredArgCount) {
        console.error(parsedCmdName + ": Expected " + command.requiredArgCount + " arguments, got " + parsedCmdArgs.length);
    } else {
        var optionalArgCount = Math.max(0, command.methodArgCount - parsedCmdArgs.length);

        try {
            var cls = new this.commandClasses[command.className](this.coreObjects);
            cls[command.methodName].apply(cls, parsedCmdArgs.concat(Array(optionalArgCount).fill(null)));
        } catch (err) {
...
Control.prototype._mapClassesToCommands = function() {
    ...
    for (var i = 0, classKeys = Object.keys(this.commandClasses); i < classKeys.length; i++) {
        ...
        for (var j = 0, methodKeys = Object.keys(cls.prototype); j < methodKeys.length; j++) {
            ...
            var methodArgCount = cls.prototype[methodName].length;
            ...
            var commandMeta = clsMeta[methodName] || clsMeta[methodName.toLowerCase()] || {};
            var commandName = prefix + (prefix && suffix? '-' : '') + suffix;

            // Check nan
            var commandArgCount = parseInt(commandMeta.requiredArgCount);
            if (commandArgCount != commandArgCount) {
                commandArgCount = methodArgCount;
            }

            commands[commandName] = {
                name: commandName,
                requiredArgCount: Math.min(methodArgCount, commandArgCount),
                className: className,
                methodName: methodName,
                methodArgCount: methodArgCount,
                meta: commandMeta
            };
        }
    }
