...
Core.prototype.help = function() {
    ...
    for (var i = 0; i < commandNames.length; i++) {
        ...
        for (var j = 0; j < command.requiredArgCount; j++) {
            argHelp.push('<arg-' + (j + 1) + '>');
        }

        for (var j = 0, m = (meta.argHelp || []); j < m.length; j++) {
            if (argHelp[j]) {
                argHelp[j] = "<" + m[j] + ">";
            } else {
                argHelp.push("[" + m[j] + "]");
        ...
        argHelp = argHelp.join(' ');

        lines.push([command.name, argHelp, meta.help || " "]);

        commandCount++;
    }

    var getLongestLine = function(col) {
        var longestLine = 0;

        for (var i = 0; i < lines.length; i++) {
            var line = lines[i][col || 0];
    ...
    for (var i = 0; i < lines[0].length; i++) {
        longestLines.push(getLongestLine(i));
    }

    for (var i = 0; i < lines.length; i++) {
        for (var j = 0; j < longestLines.length; j++) {
            var longestLine = longestLines[j];
            var padding = Array(Math.max(0, longestLine - lines[i][j].length)).fill(" ").join('');

            if (j == 0) {
                lines[i][j] = padding + lines[i][j];
            } else {
                lines[i][j] = lines[i][j] + padding;
            }
        }
    }
    ...
    console.info();
    console.info("TOTAL:", commandCount);
};
...
Bot.prototype.add = function(username, password, proxy) {
    var bot = this.mgr.add({username: username, password: password, proxy: proxy});

    if (bot) {
        this.mgr.pushPendingReconnect(bot);
    }
};
...
Bot.prototype.disconnect = function(bot) {
    if (!(bot = this.mgr.get(bot))) {
        console.warn("No such bot");
    } else {
        bot.disconnect();
    }
...
// Docs and command config
Api.META = {
};

Bot.META = {
    add: {
        help: "Add bot to manager manually",
        requiredArgCount: 1,
        argHelp: ["username", "password", "proxy"]
    }
};
...
Core.META = {
    ...
    logReset: {
        help: "Close and reopen logs"
    },
    quit: {
        help: "Shutdown bots and manager"
    }
};

Offer.META = {
    accept: {
        help: "Accept given offer on given bot",
        argHelp: ["bot ID", "offer ID"]
    }
};
