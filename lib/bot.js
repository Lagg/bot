    ...
    this._user.on("friendRelationship", function(steamId, relationship) {
        ...
        switch (relationship) {
            case SteamUser.EFriendRelationship.RequestRecipient:
                self.log.info("Rejecting friend request from " + steamId64);
                self._user.removeFriend(steamId);
                break;
            default:
                self.log.info("Relationship with", steamId64, "changed to", relationship);
                break;
        }
    });

    this._user.on("tradeRequest", function(steamId, responseCallback) {
        self.log.info("Rejecting trade request from " + steamId.getSteamID64());
...
Bot.prototype._doLogin = function() {
    ...
    if (self.username == Bot.ANONYMOUS_USERNAME) {
...
Bot.prototype.connect = function(password, callback) {
    ...
    if (this.loggedOn || this.loggingOn) {
