...
Core.prototype.init = function(conf) {
    ...
    // Initialize bots and reconnect watcher (bootstrapping off checkReconnect)
    coreObjects.manager = new BotManager;

    coreObjects.log.info("Registering", conf.bots.length, "bots");

    conf.bots.forEach(function(bot) {
        coreObjects.manager.add(bot.username, bot.password, bot.personaName);
